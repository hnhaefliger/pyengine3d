...
- Some patterns contains faces that are not triangles. Patterns would use a uniq
color to show each rigid polygon (So 2 consecutive same color triangles do not
fold in beetween in the final model - but they can fold a bit during folding for
gaining light paper twist).
...
test = graphics.engine.Engine3D(vertices, polygons, folds, title='Fold')
